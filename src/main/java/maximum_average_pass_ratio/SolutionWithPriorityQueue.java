...
public class SolutionWithPriorityQueue {
    public double maxAverageRatio(int[][] classes, int extraStudents) {
        Queue<List<Integer>> maxHeap = new PriorityQueue<>(
                (a, b) ->
                    (int) ((getDifferenceWithNext(b.get(0), b.get(1)) - getDifferenceWithNext(a.get(0), a.get(1))) * 1e9)
        );

        Arrays.stream(classes)
                .forEach(c -> maxHeap.add(Arrays.asList(c[0], c[1])));
//        maxHeap.forEach(System.out::println);

        IntStream.range(0, extraStudents)
                .forEach(i -> {
                    List<Integer> min = maxHeap.poll();
                    min.set(0, min.get(0) + 1);
                    min.set(1, min.get(1) + 1);
                    maxHeap.add(min);
                });

        return maxHeap.stream()
                .mapToDouble(c -> (double) c.get(0) / (double) c.get(1))
                .sum() / classes.length;
    }

    private double getDifferenceWithNext(int dividend, int delimiter) {
        double result = (double) (delimiter - dividend) / ((double) delimiter * (delimiter + 1));
//        System.out.printf("Difference: %f, dividend: %d, delimiter: %d%n", result, dividend, delimiter);
        return result;
    }

    public static void main(String[] args) {
        SolutionWithPriorityQueue solution = new SolutionWithPriorityQueue();
        long start = System.currentTimeMillis();
        // 0.61455
        System.out.println(solution.maxAverageRatio(new int[][]{new int[]{13609,17094}, new int[]{24079,89827}}, 22159));
        // 0.6017073671169322
